...
public class AdjustEditor : AssetPostprocessor
{
    ...
    private static void RunPostBuildScript(BuildTarget target, string projectPath = "")
    {
        if (target == BuildTarget.iOS)
        {
            ...
            if (xcodeProject.ContainsFileByProjectPath("Libraries/Plugins/iOS/AdjustSigSdk.a"))
            {
                if (!string.IsNullOrEmpty(xcodeTargetUnityFramework))
                {
                    xcodeProject.AddBuildProperty(xcodeTargetUnityFramework, "OTHER_LDFLAGS", "-force_load $(PROJECT_DIR)/Libraries/Plugins/iOS/AdjustSigSdk.a");
                }
                else
                {
                    xcodeProject.AddBuildProperty(xcodeTarget, "OTHER_LDFLAGS", "-force_load $(PROJECT_DIR)/Libraries/Plugins/iOS/AdjustSigSdk.a");
                }
            }
